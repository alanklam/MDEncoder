...
def generate_pdb(samples,pdbname,namelist,beta=[0]):
    N_frame , N_atom = np.shape(samples)[0], np.shape(samples)[1]//3
    if len(beta) != N_frame:
        beta = np.zeros(N_frame)
    with open(pdbname, 'w') as pdb:
        pdb.write('REMARK generated by MDEncoder\n')
        for i in range(N_frame):
            for j in range(N_atom):
                seg, resname, resid, atom, _ =  namelist[3*j].split('_')
                pdb.write("ATOM{0:>7d}{1:>5s} {2:>4s}{3:>1.1s}{4:>4s}{5:12.3f}{6:8.3f}{7:8.3f}{8:6.2f}{9:6.2f}{10:>7s}{11:>5.1s}\n".
                          format(j+1,atom,resname,seg,resid,samples[i,3*j],samples[i,3*j+1],samples[i,3*j+2],1.00,beta[i],seg,atom))
