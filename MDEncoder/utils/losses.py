...
def embedded_loss(f_dim,l_dim): 
    def embedded_loss(y_true, y_pred):
        # pairwise squared distance loss, for multidimensional scaling
        na = tf.reduce_sum(tf.square(y_true), 1)
        nb = tf.reduce_sum(tf.square(y_true), 1)
...
def RV_metric(y_true, y_pred):
    ...
    norm = tf.reduce_sum(tf.square(y_true), 1)    
    # na as a row and nb as a column vectors
    na = tf.reshape(norm, [-1, 1])
    nb = tf.reshape(norm, [1, -1])
    # return pairwise euclidead difference matrix
    dist_raw = tf.sqrt(tf.maximum(na - 2*tf.matmul(y_true, y_true, False, True) + nb, 0.0))
